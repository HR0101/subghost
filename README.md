...
- ログイン時起動、ホバー展開、フルスクリーン表示、自動収納を設定可能
...
- 質問の選択肢はセッション記録から復元してノッチに表示し、押せば回答できる
  - 回答の送信にはキー入力の合成を使うため、**アクセシビリティの許可**が必要（設定 > 一般 > キー入力の送信）
...
毎回 `tmux` を実行するのを忘れたくない場合は、設定の「統合」→「自動でtmux内起動」を有効にする。以後は `claude` / `codexA` / `codexB` / `agyy` を普通に起動するだけで、自動的にtmux内で立ち上がる。`codexA`・`codexB`・`agyy`の既存エイリアスとオプションはそのまま保持される。tmux外のセッションをSubghostが検出した場合も、ノッチのセッション一覧からこの設定を有効にできる。
...
設定の「一般」では、常駐アプリとしての細かな挙動を変更できる。

- macOSへのログイン時にSubghostを自動起動
- ホバー展開のオン/オフ、誤反応を防ぐ待ち時間、展開アニメーションの速度
- プロンプト入力を開くグローバルショートカット（`⌥Space` / `⌃Space` / `⇧⌘Space`）
- 対象のターミナルタブを見ているとき、完了パネルだけを自動展開しないスマート抑制
- フルスクリーン時、またはAIセッションがないときの自動非表示
- 完了パネルの表示時間、マウス離脱・外側クリックによる収納

承認待ち・質問・プロンプト入力中は、見逃しや操作不能を防ぐため自動非表示の対象外となる。スマート抑制中もmacOS通知とサウンドは通常どおり届く。
